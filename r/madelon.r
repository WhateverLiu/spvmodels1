...
if(T)
{
  ...
  pdf("figure/dtreeMadelonCp.pdf", width = 9, height = 9 * 0.618)
  ...
  pdf("figure/dtreeLearningCurveMadelon.pdf", width = 9, height = 9 * 0.618)
  ...
  legend("topright", legend = c("Cross validation error", "Training error", "Full data CV error = 0.177"), col = c("darkblue", "red", "white"), cex = 2, lwd = c(2, 2, 1), bty = "n")
  lines(x = dataSizes, y = tmp$validErr, col = "darkblue", lwd = 2)
  dev.off()
  save(dtreeLearned, file = "data/dtreeLearingCurveMadelon.Rdata")
...
if(T)
{
  ...
  if(T)
  {
    ...
    rm(tmpdat); gc()
    save(rst, file = "data/xgboostRstMadelon.Rdata")
    ...
    pdf("figure/xgboostLrNtreeMadelon.pdf", width = 10, height = 8)
    par(mar = c(4, 5, 0, 0), family = "serif")
...
if(T)
{
  ...
  if(T)
  {
    ...
    snow::stopCluster(cl) 
    save(rbfKernelRst, file = "data/rbfKernelRstMadelon.Rdata")
    
    
    pdf("figure/rbfKernelRstCvErrInitialMadelon.pdf", width = 9, height = 9 * 0.618)
    par(mar = c(4, 5, 0, 0), family = "serif")
  ...
  if(T)
  {
    ...
    snow::stopCluster(cl) 
    save(lapKernelRst, file = "data/lapKernelRstMadelon.Rdata")
    
    
    pdf("figure/lapKernelRstCvErrInitialMadelon.pdf", width = 9, height = 9 * 0.618)
    par(mar = c(4, 5, 0, 0), family = "serif")
    ...
    axis(side = 1, at = 1:length(tmp[[1]]), labels = lambda, cex.axis = 1.5)
    linecols = c("black", "red", "blue", "olivedrab3")
    legend("left", legend = c(expression(sigma == 0.1), expression(sigma == 0.01), expression(sigma == 0.001), expression(sigma == 0.0001)), col = linecols, pch = c(15, 15, 15, 15), cex = 1.5, bty = "n")
    for(i in 1:length(tmp))
    {
      lines(tmp[[i]], col = linecols[i], lwd = 2)
    }
    legend("bottomleft", legend = paste0("min(err) = ", round(min(unlist(tmp)), 3)), cex = 1.5, bty = "n")
    dev.off()
  ...
  # min(err) = 0.3976923, opt(lambda) = 9, opt(sigma) = 0.001, Laplacian kernel.
  # Plot learning curve.
  if(T)
  {
    ...
    trainValidErr = as.data.frame(t(as.data.frame(lapply(pieceList, function(x)
    {
      cat(".")
      alldat = dat[x, ]
      lapX = kernlab::kernelMatrix(kernlab::laplacedot(sigma = 0.001), as.matrix(alldat[-ncol(alldat)]))
      set.seed(42)
      mdl = kernlab::ksvm(x = lapX, y = as.factor(alldat$y), scaled = F, type = "C-svc", kernel = "matrix", C = 9, cross = 10, cache = 2048)
    ...
    learningCurve = data.frame(dataSize = unlist(lapply(pieceList, function(x) length(x))), trainValidErr)
    tmp = learningCurve
    pdf("figure/lapKernelSVMlearningCurveMadelon.pdf", width = 9, height = 9 * 0.618)
    par(mar = c(4, 5, 0, 0), family = "serif")
    plot(x = tmp$dataSize, y = tmp$trainErr, ylim = range(unlist(tmp[-1])), type = "l", col = "red", xlab = "N(observation)", ylab = "Error", bty = "L", cex.lab = 2, cex.axis = 1.5, lwd = 2)
    legend("bottomright", legend = c("Cross validation error", "Training error"), col = c("darkblue", "red"), cex = 2, lwd = c(2, 2), bty = "n")
    lines(x = tmp$dataSize, y = tmp$validErr, col = "darkblue", lwd = 2)
    dev.off()
    save(learningCurve, file = "data/lapKernelSVMlearingCurveMadelon.Rdata")
  }
...
if(T)
{
  ...
  pdf("figure/cvErrKNNmadeLon.pdf", width = 9, height = 9 * 0.618)
  par(mar = c(4, 5, 0, 0), family = "serif")
  ...
  legend("topright", legend = c("min(err) = 0.238", "opt(K) = 35"), bty = "n", cex = 2)
  dev.off()
  
  
  tmp = unlist(lapply(knnCVs, function(x) min(colMeans(x))))
  pdf("figure/KNNlearningCurveMadelon.pdf", width = 9, height = 9 * 0.618)
  par(mar = c(4, 5, 0, 0), family = "serif")
...
if(T)
{
  ...
  if(F)
  {
    ...
    tmpX = apply(tmpX, 2, function(x) x - min(x))
    tmp = max(tmpX)
    tmpX = apply(tmpX, 2, function(x) x / tmp)
    trainX = tmpX
    trainY = as.integer(round((dat$y + 1L) / 2L))
    ...
    opt = keras::optimizer_adam(lr = 1e-4) #, decay = 1 - 0.999965876413334)
    keras::compile(model, 
                   loss = "binary_crossentropy",
                   # loss = "sparse_categorical_crossentropy", 
                   optimizer = opt, metrics = "accuracy")
    system.time({fm = keras::fit(
      model, x = trainX, y = trainY, batch_size = 32, epochs = 300, 
  ...
  if(T)
  {
    ...
    for(i in 1:length(valiIndList))
    {
      ...
      keras::compile(model, loss = "binary_crossentropy", optimizer = opt, metrics = "accuracy")
      system.time({fm = keras::fit(model, x = trainX, y = trainY, batch_size = 32, epochs = 2000, validation_data = list(validX, validY), verbose = 2, shuffle = T)}) # https://keras.rstudio.com/reference/fit.html
      model1cv[[i]] = fm
    }
    save(model1cv, file = "data/model1cv.Rdata")
    ...
    for(i in 1:length(valiIndList))
    {
      ...
      opt = keras::optimizer_adam(lr = 0.0001)
      keras::compile(model, loss = "binary_crossentropy", optimizer = opt, metrics = "accuracy")
      system.time({fm = keras::fit(model, x = trainX, y = trainY, batch_size = 32, epochs = 1000, validation_data = list(validX, validY), verbose = 2, shuffle = T)}) # https://keras.rstudio.com/reference/fit.html
      model2cv[[i]] = fm
    }
    save(model2cv, file = "data/model2cv.Rdata")
  ...
  if(T)
  {
    ...
    if(T)
    {
      ...
      pdf("figure/neuralnetMadelon.pdf", width = 11, height = 11 * 0.25)
      par(mar = c(4.2, 5, 0, 0), mfrow = c(1, 4), family = "serif")
      lossylim = range(c(range(model1cvMean$trainLoss), range(model1cvMean$validLoss), range(model2cvMean$trainLoss), range(model2cvMean$validLoss)))
      plot(model1cvMean$trainLoss, ylim = lossylim, type = "l", col = "red", bty = "L", cex.lab = 2, cex.axis = 1.5, xlab = "Epoch", ylab = "Loss")
      lines(model1cvMean$validLoss, type = "l", col = "darkblue")
      legend("top", legend = "Model 1", bty = "n", cex = 1.5)
      legend("bottomleft", legend = c("Train mean", "CV mean"), bty = "n", cex = 1.5, col = c("red", "darkblue"), pch = c(15, 15))
      ...
      errylim = range(c(range(model1cvMean$trainErr), range(model1cvMean$validErr), range(model2cvMean$trainErr), range(model2cvMean$validErr)))
      plot(model1cvMean$trainErr, ylim = errylim, type = "l", col = "skyblue", bty = "L", ylab = "Classification error", xlab = "", cex.lab = 2, cex.axis =1.5)
      lines(model1cvMean$validErr, type = "l", col = "olivedrab3")
      legend("top", legend = "Model 1", bty = "n", cex = 1.5)
      legend("bottomleft", legend = c("CV mean", "Train mean"), bty = "n", cex = 1.5, col = c("olivedrab3", "skyblue"), pch = c(15, 15))
      legend("right", legend = "min(CV) = 0.388", bty = "n", cex = 1.5)
      ...
      legend("top", legend = "Model 2", bty = "n", cex = 1.5)
      legend("right", legend = "min(CV) = 0.370", bty = "n", cex = 1.5)
  ...
  if(T)
  {
    ...
    for(i in 1:length(pieceList))
    {
      ...
      opt = keras::optimizer_adam(lr = 0.0001)
      keras::compile(model, loss = "binary_crossentropy", optimizer = opt, metrics = "accuracy")
      system.time({fm = keras::fit(model, x = trainX, y = trainY, batch_size = 32, epochs = 1000, validation_split = 0.3, verbose = 0, shuffle = T)}) # https://keras.rstudio.com/reference/fit.html
      modelsForTrainSizes[[i]] = fm
    }
    
    
    trainErr = unlist(lapply(modelsForTrainSizes, function(x) 1 - mean(tail(x$metrics$accuracy, n = 250))))
    validErr = unlist(lapply(modelsForTrainSizes, function(x) 1 - mean(tail(x$metrics$val_accuracy, n = 250))))
    pdf("figure/neuralnetMadelonLearningCurveAgainstDataSize.pdf", width = 9, height = 9 * 0.618)
